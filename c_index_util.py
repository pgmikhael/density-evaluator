...
def get_censoring_dist(times: list, event_observed: list) -> dict:
    """
    Obtain the censoring distribution for each observed time.

    Parameters
    ----------
    times : list
        list of observed times, right censored
    event_observed : list
        list of event observed

    Returns
    -------
    dict
        dictionary of censoring distribution for each observed time
    """
...
def _concordance_summary_statistics(
    ...
    for time in observed_times:
        times_to_compare[time] = _BTree(np.unique(died_pred))
    ...
    while True:
        ...
        if has_more_censored and (
            ...
            for pred in died_pred[died_ix:next_ix]:
                for time in observed_times:
                    times_to_compare[time].insert(pred)
            died_ix = next_ix
        else:
...
def _handle_pairs(truth, pred, first_ix, times_to_compare, censoring_dist):
    ...
    for i in range(first_ix, next_ix):
        rank, count = times_to_compare[truth_time].rank(pred[i])
        correct += rank
        tied += count
